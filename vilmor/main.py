...
from vilmor.base.github import get_git_current_user
from vilmor.cli import CLIConfig
from vilmor.cli.stdout import print_msg
from vilmor.config import Configuration
from vilmor.utils import generate_recipe, origin_is_github, origin_is_local_sdist
...
def init_parser():
    ...
    cran_parser.add_argument(
        ...
        help="If sections are specified, vilmor will populate just the sections "
        "informed.",
    )
    ...
    pypi_parser.add_argument(
        ...
        action="store_true",
        dest="is_recursive",
        help="Recursively run vilmor on missing dependencies.",
    )
    pypi_parser.add_argument(
        "--sections",
        "-s",
    ...
    pypi_parser.add_argument(
        ...
        help="If tag is specified, vilmor will build from release tag",
    )
    pypi_parser.add_argument(
        "--extras-require-all",
        default=False,
...
def generate_recipes_from_list(list_pkgs, args):
    for pkg_name in list_pkgs:
        logging.debug(f"Starting vilmor for pkg: {pkg_name}")
