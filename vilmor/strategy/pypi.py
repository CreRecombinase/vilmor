...
from vilmor.base.github import generate_git_archive_tarball_url, handle_gh_version
from vilmor.base.pkg_info import normalize_pkg_name
from vilmor.base.track_packages import _get_track_info_from_file, solve_list_pkg_name
from vilmor.cli.stdout import print_msg, print_requirements, progressbar_with_status
from vilmor.config import Configuration
from vilmor.strategy.abstract_strategy import AbstractStrategy
...
from vilmor.utils import format_dependencies, origin_is_github, rm_duplicated_deps
...
def get_pypi_metadata(config: Configuration) -> dict:
    ...
    if config.download:
        download_file = os.path.join(
            str(mkdtemp(f"vilmor-pypi-metadata-{config.name}-")), "pypi.json"
        )
